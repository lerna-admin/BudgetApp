...
import {
  deleteSplitBill,
  getSplitBill,
  splitBillTitleExists,
  updateSplitBill,
} from "../../../../lib/server/split-bills-repository";
...
export async function GET(_request, { params }) {
  try {
    ...
    const data = await getSplitBill(id, { userId: user.id });
    if (!data) {
      return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    }
    return NextResponse.json({ data });
  } catch (error) {
    console.error("GET /api/split-bills/[id] failed", error);
...
export async function PUT(request, { params }) {
  try {
    ...
    const duplicate = await splitBillTitleExists({ userId: user.id, title, excludeId: id });
    if (duplicate) {
    ...
    const data = await updateSplitBill(id, {
      userId: user.id,
      title,
      currency,
      payerType,
    ...
    console.error("PUT /api/split-bills/[id] failed", error);
...
export async function DELETE(_request, { params }) {
  try {
    ...
    const removed = await deleteSplitBill(id, { userId: user.id });
    if (!removed) {
      return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    }
    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("DELETE /api/split-bills/[id] failed", error);
