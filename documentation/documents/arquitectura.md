...
- Registro de movimientos: al saldarse completamente el balance del usuario ($0), el sistema muestra un banner para registrar los movimientos derivados como ingresos/egresos en el presupuesto. Se sugieren tres tipos: (1) ingreso cuando un amigo pagó al usuario, (2) gasto cuando el usuario pagó a un amigo, (3) gasto de consumo propio si el usuario pagó parte de la cuenta. El usuario completa descripción (editable), subcategoría, rubro (obligatorio si aplica), método de pago, tarjeta/banco/cuenta según método y notas. Los IDs de los movimientos creados se persisten en el campo `expenseIds` de cada liquidación en `settlements_json`, garantizando que el banner no reaparezca en ningún dispositivo. Al deshacer una liquidación con movimientos registrados, se ofrece eliminar solo los movimientos de esa liquidación.
